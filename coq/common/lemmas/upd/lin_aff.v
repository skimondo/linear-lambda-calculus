...
(* Admitted lemma corresponding to the first recursive function *)
Lemma lookup_lab_neq :
  forall {N : nat} {psi : ctx} {delta delta' delta'' : lctx N} 
         {n m : nat} {X X' Y Y' : obj} {A A' B B' : tp} 
         {alpha' beta' : mult},
...
(* Second lemma that uses the first one *)
Lemma lookup_lab_neq_2 :
  forall {N : nat} {psi : ctx} {delta delta' delta'' : lctx N} 
         {n m : nat} {X X' Y Y' : obj} {A A' B B' : tp} 
         {alpha' beta' : mult},
    upd delta n X X' A A' zero alpha' delta' ->
    upd delta m Y Y' B B' one beta' delta'' ->
    n <> m.
Admitted.
